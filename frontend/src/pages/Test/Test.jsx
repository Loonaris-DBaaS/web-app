...
const PLACEHOLDER = 'postgresql://sk_live_xxxx…_rw@<gateway-host>:5432/app?sslmode=disable';
...
export default function Test() {
  ...
  async function handleStop() {
    if (!run?.runId) return;
    try {
      await loadTestService.stop(run.runId);
    } catch {
      /* ignore */
    }
  }
    ...
    <div style={wrap}>
      <div>
        <h1
          style={{
            margin: 0,
            fontSize: 'var(--text-headline-sm-size, 28px)',
            color: 'var(--on-surface)',
          }}
        >
        ...
        >
          Paste a Loonaris connection string, start the load, and watch CPU &amp; memory respond in
          real time.
        </p>
      </div>
      ...
      <div style={card}>
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 'var(--space-3)', marginLeft: 'auto' }}>
            <button
              style={btn('var(--surface-container-low)', 'var(--on-surface)')}
            ...
            {running ? (
              ...
              <button
                style={btn('var(--primary)', 'var(--on-primary, #fff)')}
                onClick={handleStart}
                disabled={!connectionString || busy}
              >
        ...
        {error && (
          <p style={{ margin: 0, color: 'var(--error)', fontSize: 'var(--text-body-sm-size)' }}>
            {error}
          </p>
        )}
        ...
        {loadStatus && (
          ...
          >
            <span>
              Status:{' '}
              <strong style={{ color: running ? 'var(--success)' : 'var(--on-surface)' }}>
                {running ? 'running' : 'finished'}
              </strong>
            </span>
            <span>
              Rows inserted: <strong>{loadStatus.inserted.toLocaleString()}</strong>
            </span>
            <span>~{loadStatus.insertsPerSec.toLocaleString()} rows/s</span>
            <span>Elapsed: {loadStatus.elapsedSec}s</span>
            {loadStatus.errors > 0 && (
              <span style={{ color: 'var(--error)' }}>Errors: {loadStatus.errors}</span>
            )}
          </div>
        )}
      </div>
      ...
      {polling && (
        <div style={card}>
          <h2
            style={{ margin: 0, fontSize: 'var(--text-title-lg-size)', color: 'var(--on-surface)' }}
          >
            Live metrics
          </h2>
